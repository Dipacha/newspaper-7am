...
class PostDetailview(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 4, 3, 2, 1
        obj = self.get_object()
        context['previous_post'] = (Post.objects.filter(id__lt=obj.id,
        status="published",
        published_at__isnull=False,
        ).order_by("-id").first())
        ...
        #detail => 5
        # 6, 7, 8, 9
        context['next_post'] = (Post.objects.filter(id__lt=obj.id,
        status="published",
        published_at__isnull=False,
        ).order_by("id").first())

        context["recent_posts"] = Post.objects.filter(
            status="published",published_at__isnull=False).order_by("-views_count")[:4] 
...
class ContactView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Successfully submitted your message. we wil contact you soon.",
        )
        else:
